package it.tugaia56.obsidian.xposed.batterystyles
...
open class LandscapeBatteryKim(private val context: Context, frameColor: Int) :
    ...
    private fun loadPaths() {
        ...
        val errorPathString =
            getResources(context).getString(R.string.config_landscapeBatteryErrorLandscapeKim)
        errorPerimeterPath.set(PathParser.createPathFromPathData(errorPathString))
        errorPerimeterPath.computeBounds(RectF(), true)

        val fillMaskString =
            getResources(context).getString(R.string.config_landscapeBatteryFillMaskLandscapeKim)
        fillMask.set(PathParser.createPathFromPathData(fillMaskString))
        // Set the fill rect so we can calculate the fill properly
        fillMask.computeBounds(fillRect, true)

        val boltPathString =
            getResources(context).getString(R.string.config_landscapeBatteryBoltLandscapeKim)
        boltPath.set(PathParser.createPathFromPathData(boltPathString))

        val plusPathString =
            getResources(context).getString(R.string.config_landscapeBatteryPlusLandscapeKim)

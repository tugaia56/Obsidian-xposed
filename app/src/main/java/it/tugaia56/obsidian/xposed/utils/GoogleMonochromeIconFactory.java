...
/**
 * Ported verbatim from OC (pure image-processing utility, no OC-specific dependencies).
 * Generates a monochrome (Material You "themed icon" style) version of any drawable —
 * used as the fallback when the OEM's own themed-icon lookup returns null for an app.
 */
public class GoogleMonochromeIconFactory extends BitmapDrawable {
    ...
    public GoogleMonochromeIconFactory(Drawable icon, int iconBitmapSize) {
        ...
        // Create a color matrix which converts the icon to grayscale and then uses the average
